...
class SandSim:
    ...
    def __init__(self, width: int, height: int, cell_size: int = 4, fps: int = 2000) -> None:
    ...
    def update(self) -> None:
        ...
        * WATER: the same as sand, PLUS:
        ...
        #reading grid down to up
        for row in range(self.height-1, -1, -1):
            #randomizing the order of columns
            ...
            for col in cols:
                ...
                if cell == Material.SAND:
                    ...
                    if row + 1 < self.height and grid_copy[row + 1, col] == Material.EMPTY:
                        ...
                        grid_copy[row + 1, col] = Material.SAND
                    else:
                        # Check diagonals
                        directions = [-1, 1]
                        ...
                        for d in directions:
                            new_col = col + d
                            if (0 <= new_col < self.width and row + 1 < self.height and grid_copy[row + 1, new_col] == Material.EMPTY):
                                grid_copy[row, col] = Material.EMPTY
                                grid_copy[row + 1, new_col] = Material.SAND
                                moved = True
                                break
                        if not moved:
                    ...
                    if row + 1 < self.height and grid_copy[row + 1, col] == Material.EMPTY:
                        grid_copy[row, col] = Material.EMPTY
                        grid_copy[row + 1, col] = Material.WATER
                    else:
                        # Check diagonals
                        directions = [-1, 1]
                        _rng.shuffle(directions)
                        ...
                        for d in directions:
                            ...
                            if (0 <= new_col < self.width and row + 1 < self.height and 
                                ...
                                grid_copy[row, col] = Material.EMPTY
                                grid_copy[row + 1, new_col] = Material.WATER
                                moved = True
                                break
                        if not moved:
                            # Spread sideways if can't fall or slide diagonally
                            _rng.shuffle(directions)
                            for d in directions:
                                new_col = col + d
                                if (0 <= new_col < self.width and 
                                    grid_copy[row, new_col] == Material.EMPTY):
                                    grid_copy[row, col] = Material.EMPTY
                                    grid_copy[row, new_col] = Material.WATER
                                    moved = True
                                    break
                            if not moved:
